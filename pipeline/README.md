...
python triage.py --dry-run  # 只列出将处理的文件：不调用模型（不花钱）、不写盘
...
- 单实例锁：同一时间只允许一个实例运行（`.triage.lock`）；超过 2 小时的陈旧锁自动接管。
- API 调用带超时与有限重试，可用 `API_TIMEOUT` / `REVIEW_TIMEOUT` / `API_MAX_RETRIES` 调整（见 `.env.example`）。
